...
function MainLayout({ children, currentPageName, currentUser, onUserUpdate }) {
  ...
  { name: "Relatório Chat", page: "RelatorioChat", icon: BarChart3, roles: ["admin", "user"] },
  ...
  { name: "Importador de Dados", page: "ImportadorDados", icon: Upload, roles: ["admin"] },
  { name: "Importar Procedimentos", page: "ImportarProcedimentos", icon: Upload, roles: ["admin"] },
  { name: "Importar Turmas", page: "ImportarTurmas", icon: Upload, roles: ["admin"] },
  { name: "Importar Pacientes", page: "ImportarPacientes", icon: Upload, roles: ["admin"] },
      ...
      <aside className={`${isSidebarCollapsed ? 'w-16' : 'w-64'} bg-white border-r border-gray-200 flex flex-col transition-all duration-300 ease-in-out ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 md:relative fixed h-full z-20`}>
        <div className="h-16 flex items-center gap-2 border-b px-3 justify-between">
          <div className="flex items-center gap-2 overflow-hidden">
            ...
            {!isSidebarCollapsed &&
            <span className="text-gray-700 text-sm font-semibold whitespace-nowrap">CENTRO VIDA SAÚDE
              ...
              if (item.external) {
                return (
                  <a
                    ...
                    {React.createElement(item.icon, { className: "w-5 h-5 mr-3 flex-shrink-0" })}
                    <span>{item.name}</span>
                    </a>);
