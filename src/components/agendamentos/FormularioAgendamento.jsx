...
export default function FormularioAgendamento({ agendamento, dadosIniciais, todosAgendamentos, medicos, pacientes: pacientesProps, procedimentos, exames, categorias, tabelaPrecos, onSave, onClose }) {
  ...
  const buscarPrecoConsulta = useCallback((medicoId, categoriaId) => {
    ...
    if (!medico) return 0;
    const espNorm = normalizeString(medico.especialidade);
    let proc = procedimentos.find(p => normalizeString(p.nome).includes('CONSULTA') && p.especialidade && normalizeString(p.especialidade) === espNorm);
    if (!proc) proc = procedimentos.find(p => normalizeString(p.nome).includes('CONSULTA') && normalizeString(p.nome).includes(espNorm));
    if (!proc) return 0;
    const preco = tabelaPrecos.find(tp => tp.procedimento_id === proc.id && tp.categoria_id === categoriaId);
  ...
  useEffect(() => {
    ...
    const initializeFormDataAndPatient = async () => {
      let initialFormData = {
        ...
        horario: '',
        tipo_servico: 'Consulta', duracao_minutos: '',
        itens_servico: [], is_encaixe: false,
        is_recorrente: false, recorrencia_tipo: '', recorrencia_data_fim: '',
        procedimento_id: '',
        exames_ids: [],
        categoria_preco_id: '',
        valor_total: '0',
