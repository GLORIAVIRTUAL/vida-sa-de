...
import { AuditoriaMedico, Medico, CategoriaPreco } from '@/entities/all';
...
const NOMES_CAMPOS = {
  ...
  percentual_repasse: 'Percentual Repasse (Particular)',
  percentual_repasse_convenio: 'Percentual Repasse (Convênio)',
  valor_repasse_fixo: 'Valor Fixo (Particular)',
  valor_repasse_fixo_convenio: 'Valor Fixo (Convênio)',
  repasses_por_categoria: 'Repasses por Categoria',
...
function formatarValorDetalhado(valor, categoriasMap) {
    ...
    if (valor[0]?.categoria_id) {
      return (
        <ul className="text-xs space-y-0.5">
          {valor.map((r, i) => {
            const nomeCat = categoriasMap[r.categoria_id] || `Cat. ${r.categoria_id?.substring(0, 8)}`;
            const simbolo = r.tipo_repasse === 'percentual' ? '%' : 'R$';
            return <li key={i}>• {nomeCat}: {simbolo} {r.valor || 0}</li>;
          })}
        </ul>
      );
    }
    if (valor[0]?.dia_semana !== undefined) {
      return <span className="text-xs">{valor.length} horário(s) configurado(s)</span>;
    }
...
function LinhaAuditoria({ registro, categoriasMap }) {
  ...
  return (
    <>
      <TableRow className="cursor-pointer hover:bg-gray-50" onClick={() => setExpandido(!expandido)}>
        ...
        <TableCell className="font-medium">{registro.medico_nome || '-'}</TableCell>
        <TableCell>
                  ...
                  <div className="grid grid-cols-2 gap-3 text-sm">
                    <div>
                      ...
                      <div className="bg-red-50 p-2 rounded border border-red-100">
                        {formatarValorDetalhado(c.valor_antigo, categoriasMap)}
                      </div>
                    </div>
                    <div>
                      <span className="text-xs text-green-600 font-medium block mb-1">DEPOIS:</span>
                      <div className="bg-green-50 p-2 rounded border border-green-100">
                        {formatarValorDetalhado(c.valor_novo, categoriasMap)}
                      </div>
                    </div>
                  </div>
                </div>
...
export default function AuditoriaMedicos() {
  ...
  const carregar = async () => {
    setLoading(true);
    try {
      const [audData, medData, catData] = await Promise.all([
        AuditoriaMedico.list('-created_date', 500),
        Medico.list(),
        CategoriaPreco.list()
      ]);
      ...
      setMedicos(medData || []);
      const map = {};
      (catData || []).forEach(c => { map[c.id] = c.nome; });
  ...
  const registrosFiltrados = useMemo(() => {
    return registros.filter(r => {
      if (filtroMedico !== 'todos' && r.medico_id !== filtroMedico) return false;
  ...
  }, [registros, filtroMedico, filtroEvento, filtroDataInicio]);
          ...
          <div className="mb-6 flex justify-between items-start">
            <div>
              <h1 className="text-3xl font-bold text-gray-900 mb-2 flex items-center gap-2">
                ...
                Auditoria de Médicos
              </h1>
              <p className="text-gray-600">
                Histórico de todas as alterações nos cadastros dos médicos.
              </p>
            </div>
            ...
            <CardContent>
              {loading ? (
                ...
                <div className="text-center py-12 text-gray-500">
                  ...
                  <p className="text-sm mt-1">As alterações futuras nos cadastros de médicos aparecerão aqui.</p>
                </div>
              ) : (
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {registrosFiltrados.map(r => (
                        <LinhaAuditoria key={r.id} registro={r} categoriasMap={categoriasMap} />
                      ))}
                    </TableBody>
                  </Table>
                </div>
